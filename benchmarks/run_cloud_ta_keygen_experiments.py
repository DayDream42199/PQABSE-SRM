...
def append_run_row(csv_path: Path, attribute_count: int, run_index: int, duration_ms: float) -> None:
    ...
    with csv_path.open("a", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow({
            ...
            "experiment": "ta_key_generation",
            "metric_name": "keygen_ms",
            "parameter_name": "attribute_count",
            "parameter_value": attribute_count,
            "run_index": run_index,
            "duration_ms": f"{duration_ms:.3f}",
        })


def append_average_row(csv_path: Path, attribute_count: int, run_values: list[float]) -> None:
    ensure_parent(csv_path)
    fieldnames = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run only the TA key-generation cloud benchmark and append CSV rows.")
    parser.add_argument("--ta-url", required=True, help="Base TA URL, for example http://1.2.3.4:8081")
    parser.add_argument("--repeats", type=int, default=DEFAULT_REPEATS)
    parser.add_argument("--averages-csv", type=Path, default=Path("cloud_ta_keygen_results.csv"))
    parser.add_argument("--runs-csv", type=Path, default=Path("cloud_ta_keygen_runs.csv"))
...
def main() -> int:
    ...
    for attribute_count in ATTRIBUTE_COUNTS:
        ...
        for run_index in range(1, args.repeats + 1):
            print(f"[ta_key_generation] attribute_count={attribute_count} run {run_index}/{args.repeats}", flush=True)
            gid = f"bench_keygen_{attribute_count}_{run_index}_{random_suffix()}"
            response = http_post_json(f"{ta_url}/mobile/register", {"gid": gid, "attributes": attribute_list(attribute_count)})
            duration_ms = require_timing(response, "keygen_ms")
            run_values.append(duration_ms)
            append_run_row(args.runs_csv, attribute_count, run_index, duration_ms)
            print(f"  -> {duration_ms:.3f} ms", flush=True)
            time.sleep(0.2)
        avg = average(run_values)
        append_average_row(args.averages_csv, attribute_count, run_values)
        print(f"[ta_key_generation] attribute_count={attribute_count} average={avg:.3f} ms", flush=True)
